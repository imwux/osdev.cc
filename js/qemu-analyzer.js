function parse_qemu_log(log) {
    ...
    while (i < lines.length) {
        ...
        if (line.startsWith("CPU Reset")) {
            ...
            if (cpuMatch) {
                ...
                while (
                    i < lines.length &&
                    lines[i].trim() !== "" &&
                    !lines[i].startsWith("CPU Reset") &&
                    !lines[i].startsWith("Servicing hardware INT") &&
                    !lines[i].startsWith("check_exception")
                ) {
        ...
        if (line.startsWith("Servicing hardware INT")) {
            const intMatch = line.match(
                /Servicing hardware INT=0x([0-9a-fA-F]+)/,
            );
            if (intMatch) {
                const event = {
                    type: "hardware_int",
                ...
                if (i < lines.length) {
                    const numLine = lines[i].trim();
                    const numMatch = numLine.match(
                        /^\s*(\d+):\s+v=([0-9a-fA-F]+)\s+e=([0-9a-fA-F]+)\s+i=(\d+)\s+cpl=(\d+)\s+IP=([0-9a-fA-F]+):([0-9a-fA-F]+)\s+pc=([0-9a-fA-F]+)\s+SP=([0-9a-fA-F]+):([0-9a-fA-F]+)\s+env->regs\[R_EAX\]=([0-9a-fA-F]+)/,
                    );
                    if (numMatch) {
                        event.sequence = parseInt(numMatch[1]);
                        event.vector = numMatch[2];
    ...
    const filtered_events = log_events.filter(
        (event) => event.type !== "hardware_int",
    );
...
function parseRegisterLine(line, registers) {
    ...
    const segMatch = line.match(
        /^([A-Z]+)\s+=([0-9a-fA-F]+)\s+([0-9a-fA-F]+)\s+([0-9a-fA-F]+)\s+([0-9a-fA-F]+)(\s+.*)?$/,
    );
    if (segMatch) {
    ...
    const dtMatch = line.match(
        /^(GDT|IDT)=\s+([0-9a-fA-F]+)\s+([0-9a-fA-F]+)$/,
    );
    if (dtMatch) {
        registers[dtMatch[1]] = {
            base: dtMatch[2],
...
const EXCEPTION_INFO = {
    "00": {
        ...
        description:
            "Occurs when dividing by zero or when the quotient is too large for the destination.",
        commonCauses: [
        ...
        howToFix:
            "Check divisor values before division operations. Ensure quotient fits in destination register.",
    },
    "01": {
        name: "Debug Exception (#DB)",
        description:
            "Triggered by debug events such as instruction breakpoints, data breakpoints, or single-step execution.",
        commonCauses: [
        ...
        howToFix:
            "This is usually intentional when debugging. Check if you accidentally enabled debug features.",
    },
    "02": {
        name: "NMI Interrupt",
        description:
            "Non-Maskable Interrupt - a high-priority hardware interrupt that cannot be disabled.",
        commonCauses: [
        ...
        howToFix:
            "Investigate hardware issues. This usually indicates a serious hardware problem.",
    },
    "03": {
        name: "Breakpoint (#BP)",
        description:
            "Generated by the INT3 instruction, used for software breakpoints in debuggers.",
        commonCauses: [
        ...
        howToFix:
            "This is normal during debugging. Remove INT3 instructions if unintended.",
    },
    "04": {
        name: "Overflow (#OF)",
        description:
            "Occurs when the INTO instruction is executed and the overflow flag (OF) is set.",
        commonCauses: [
        ...
        howToFix:
            "Check arithmetic operations for overflow conditions. Avoid INTO instruction unless needed.",
    },
    "05": {
        name: "BOUND Range Exceeded (#BR)",
        description:
            "The BOUND instruction detected that an array index is out of bounds.",
        commonCauses: [
        ...
        howToFix:
            "Validate array indices before access. Consider using modern bounds checking methods.",
    },
    "06": {
        name: "Invalid Opcode (#UD)",
        description:
            "The processor encountered an instruction it doesn't recognize or that is not valid in the current mode.",
        commonCauses: [
        ...
        howToFix:
            "Verify instruction pointer (EIP/RIP). Check for memory corruption. Ensure CPU feature support.",
    },
    "07": {
        name: "Device Not Available (#NM)",
        description:
            "Attempted to use x87 FPU, MMX, or SSE instructions when the coprocessor is not available.",
        commonCauses: [
        ...
        howToFix:
            "Initialize FPU/SSE properly. Clear TS flag with CLTS instruction. Check CR0 register settings.",
    },
    "08": {
        name: "Double Fault (#DF)",
        description:
            "A second exception occurred while handling another exception. This is a critical error.",
        commonCauses: [
        ...
        howToFix:
            "Verify IDT is properly initialized. Ensure exception handlers are valid. Check stack setup. This often indicates IDT or stack issues.",
    },
    "09": {
        name: "Coprocessor Segment Overrun",
        description:
            "Legacy exception for x87 FPU segment overrun. Not used on modern processors (486+).",
        commonCauses: [
            "Only on very old processors",
            "x87 FPU operand crosses segment boundary",
        ],
        howToFix:
            "Should not occur on modern CPUs. Check if running on ancient hardware emulation.",
    },
    "0a": {
        name: "Invalid TSS (#TS)",
        description:
            "Task State Segment (TSS) is invalid during a task switch or when loading segment registers.",
        commonCauses: [
        ...
        howToFix:
            "Verify TSS descriptor in GDT. Ensure TSS is properly initialized with correct size and type.",
    },
    "0b": {
        name: "Segment Not Present (#NP)",
        description:
            "Tried to load or use a segment register with a descriptor marked as not present.",
        commonCauses: [
        ...
        howToFix:
            "Set Present bit in segment descriptors. Ensure segments are properly loaded before use.",
    },
    "0c": {
        name: "Stack-Segment Fault (#SS)",
        description:
            "Stack operation exceeded stack segment limit or stack segment is not present.",
        commonCauses: [
        ...
        howToFix:
            "Increase stack size. Check stack pointer initialization. Verify SS segment descriptor is valid.",
    },
    "0d": {
        name: "General Protection Fault (#GP)",
        description:
            "Generic protection violation - many security and privilege checks failed.",
        commonCauses: [
        ...
        howToFix:
            "Check segment selectors and descriptors. Verify privilege levels. Ensure valid memory access patterns.",
    },
    "0e": {
        name: "Page Fault (#PF)",
        description:
            "Attempted memory access that violates page-level protection or references a non-present page.",
        commonCauses: [
        ...
        howToFix:
            "Check CR2 register for faulting address. Map required memory pages. Verify page table setup. Check for buffer overruns.",
    },
    10: {
        ...
        howToFix:
            "Check FPU status word. Validate floating-point operations. Initialize FPU properly with FINIT.",
    },
    11: {
        ...
        howToFix:
            "Align data structures properly. Disable alignment checking if not needed. Use aligned memory accesses.",
    },
    12: {
        name: "Machine Check (#MC)",
        description:
            "Critical hardware error detected by the processor or system.",
        commonCauses: [
        ...
        howToFix:
            "Check Machine Check registers. Investigate hardware issues. This is usually a serious hardware problem.",
    },
    13: {
        ...
        howToFix:
            "Check MXCSR register for exception flags. Mask unwanted exceptions in MXCSR. Validate SSE operations.",
    },
    14: {
        name: "Virtualization Exception (#VE)",
        description:
            "EPT violation that cannot be handled conventionally, delivered to guest.",
        commonCauses: [
        ...
        howToFix:
            "Check virtualization configuration. Verify EPT page tables. Used primarily in hypervisors.",
    },
    15: {
        name: "Control Protection Exception (#CP)",
        description:
            "Control-flow protection violation (CET - Control-flow Enforcement Technology).",
        commonCauses: [
        ...
        howToFix:
            "Ensure proper shadow stack usage. Add ENDBRANCH instructions. Check for control flow integrity issues.",
    },
};
...
function updateSummary() {
    ...
    document.querySelector("#total-events .summary-value").textContent =
        allEvents.length;
    document.querySelector("#exceptions-count .summary-value").textContent =
        exceptions;
...
function createEventCard(event, index) {
    ...
    if (event.type === "exception") {
        const exceptionName =
            EXCEPTION_NAMES[event.vector] || `Exception 0x${event.vector}`;
        title = exceptionName;
...
function renderExceptionDetails(event) {
    ...
    const exceptionName = exceptionInfo
        ? exceptionInfo.name
        : `Unknown Exception`;
...
function renderRegisters(registers) {
    ...
    const gpRegData = gpRegs
        .filter((reg) => registers[reg])
        .map((reg) => ({
            name: reg,
            value: registers[reg],
        }));
